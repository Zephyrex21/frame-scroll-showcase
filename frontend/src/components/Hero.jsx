...
// FIX #12: buildTransform moved OUTSIDE component — no recreation on every render
function buildTransform(base = '', y) {
  if (!base || base === 'none') return `translateY(${y}px)`;
...
function getPhaseStyle(phase, frame) {
  ...
  const y = (((frame - frameStart) / span) - 0.5) * -18;
...
const PHASES = [
  ...
  },
  {
    ...
    render: () => (
      <div>
        ...
        <p style={{ fontFamily:'var(--font-display)', fontSize:'clamp(36px,5vw,64px)', fontWeight:300, color:'var(--hero-text)', lineHeight:1.1, marginBottom:'6px' }}>346 Components.</p>
        <p style={{ fontFamily:'var(--font-display)', fontStyle:'italic', fontSize:'clamp(36px,5vw,64px)', fontWeight:300, color:'var(--hero-gold)', lineHeight:1.1, marginBottom:'24px' }}>One Movement.</p>
    ...
    render: () => (
      <div>
        ...
        <p style={{ fontFamily:'var(--font-display)', fontSize:'clamp(28px,4vw,52px)', fontWeight:300, color:'var(--hero-text)', lineHeight:1.15, marginBottom:'24px' }}>
          Every gear<br /><em style={{ color:'var(--hero-gold)' }}>set by hand.</em><br />
          Measured in<br /><em style={{ color:'var(--hero-gold)' }}>microns.</em>
        </p>
  ...
  },
  {
    ...
    render: () => (
      <div style={{ background:'rgba(8,6,4,0.75)', border:'1px solid var(--hero-border)', padding:'32px 40px', backdropFilter:'blur(6px)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px 28px' }}>
          {[['Movement','A-52 Self-Winding'],['Jewels','31 Rubies'],['Frequency','28,800 bph'],['Power Reserve','72 Hours'],['Diameter','40 mm'],['Chronometer','COSC Certified']].map(([l,v]) => (
    ...
    render: () => (
      <div>
        ...
        <p style={{ fontFamily:'var(--font-display)', fontSize:'clamp(30px,4.2vw,56px)', fontWeight:300, color:'var(--hero-text)', lineHeight:1.15, marginBottom:'24px' }}>
          Brought together<br />in <em style={{ color:'var(--hero-gold)' }}>eleven weeks.</em>
        </p>
        <p style={{ fontFamily:'var(--font-body)', fontSize:'13px', lineHeight:1.8, color:'var(--hero-muted)', fontWeight:300 }}>
          Each Imperion requires 11 weeks of individual construction. Inspected at 23 quality-control stations before leaving Geneva.
        </p>
  ...
  },
  {
    ...
    render: (watch) => (
      <div>
        <p style={{ fontFamily:'var(--font-display)', fontStyle:'italic', fontSize:'clamp(36px,5vw,64px)', fontWeight:300, color:'var(--hero-text)', letterSpacing:'0.04em', marginBottom:'6px' }}>Imperion</p>
        ...
        <button
          ...
          style={{ fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'0.22em', textTransform:'uppercase', color:'#080604', background:'var(--hero-gold)', border:'none', padding:'14px 40px', cursor:'none' }}
          data-cursor
...
export default function Hero({ watch }) {
  ...
  const ctxRef          = useRef(null);   // FIX #3: cache canvas context
  const heroRef         = useRef(null);
  const imagesRef       = useRef([]);
  const currentFrameRef = useRef(0);
  const phaseEls        = useRef([]);
  const rafRef          = useRef(null);
  const heroTopRef      = useRef(0);      // FIX #7: cache hero offsetTop
  const loadCountRef    = useRef(0);      // FIX #4: ref for load count, avoid 240 re-renders
  ...
  const drawFrame = useCallback((index) => {
    ...
    const ctx    = ctxRef.current;        // FIX #3: use cached context
  ...
  /* ── Update text overlay opacities via direct DOM (zero React re-renders) ─ */
  ...
  /* ── Canvas resize — debounced, caches context ───────────────────────────── */
  useEffect(() => {
    let timer;
    const resize = () => {
      clearTimeout(timer);
      timer = setTimeout(() => {                 // FIX #14: debounce resize
        const c = canvasRef.current;
        if (!c) return;
        c.width  = window.innerWidth;
        c.height = window.innerHeight;
        ctxRef.current = c.getContext('2d', { alpha: false }); // FIX #3: cache ctx after resize
        drawFrame(currentFrameRef.current);
        heroTopRef.current = heroRef.current?.offsetTop ?? 0;  // FIX #7: recache after resize
      }, 100);
    };
    resize();
  ...
  useEffect(() => {
    ...
    const load = (i) => new Promise(resolve => {
      ...
      img.onload = img.onerror = () => {
        ...
        // FIX #4: Throttle setState — update UI every 8 frames, not every frame
      ...
      // FIX #1: Relative URL — uses Vite proxy in dev, same-origin in prod
  ...
  /* ── Cache hero position after mount ─────────────────────────────────────── */
  useEffect(() => {
    heroTopRef.current = heroRef.current?.offsetTop ?? 0; // FIX #7
  }, []);
    ...
    const onScroll = () => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        const progress = Math.max(0, Math.min(1,
          (window.scrollY - heroTopRef.current) / SCROLL_HEIGHT  // FIX #7: use cached value
        ));
  ...
  return (
    <section ref={heroRef} id="hero"
      ...
      {/* Sticky viewport — FIX #8: will-change promotes to GPU compositor layer */}
      <div style={{
        ...
        {/* Canvas — FIX #8: will-change for GPU layer */}
        ...
        <div style={{
          ...
          background:`
            linear-gradient(to right,  rgba(8,6,4,0.5) 0%, transparent 32%),
            linear-gradient(to left,   rgba(8,6,4,0.5) 0%, transparent 32%),
            linear-gradient(to top,    rgba(8,6,4,0.6) 0%, transparent 28%),
            linear-gradient(to bottom, rgba(8,6,4,0.65) 0%, transparent 32%)
          `,
        }} />
        ...
        {PHASES.map((phase, i) => (
          <div
            ...
            style={{
              ...
              padding: '0 40px',
              ...Object.fromEntries(
                Object.entries(phase.pos).filter(([k]) => k !== 'transform' && k !== 'maxWidth' && k !== 'textAlign')
              ),
        ...
        {/* Loading screen */}
        {!isLoaded && (
          <div style={{ position:'absolute', inset:0, background:'var(--hero-bg)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'32px', zIndex:10 }}>
            ...
            <div style={{ width:'220px' }}>
              <div style={{ height:'1px', background:'rgba(201,151,58,0.15)', overflow:'hidden' }}>
                <div style={{ height:'1px', width:`${loadProgress}%`, background:'var(--hero-gold)', transition:'width 0.2s ease' }} />
              </div>
